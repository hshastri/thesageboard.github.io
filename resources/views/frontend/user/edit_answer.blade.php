...
@section('user_content')
    <div class="row">
        <div class="col-md-12 mb-5 question-details">
            <form method="post" action="{{ route('answer-edit', base64_encode($answer->id)) }}">
                @csrf
                <h2 class="mb-4"><a href="{{route('question-details', base64_encode($question->id))}}">{{$question->question_title}}</a></h2>
                <div class="form-group row">
                    <div class="col-lg-12">
                        <textarea id="answer-details" aria-required="true" cols="58" rows="8" name="answer" required>{{@$answer->answer}}</textarea>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-12 pull-left">
                        <button type="submit" class="btn btn-success pull-left">Update Answer</button>
                    </div>
