...
<form action="{{route('release-refund' , $refund->id)}}" method="post">
    @csrf
    <input type="hidden" name="transaction_id" value="{{$refund->payment_id}}">
    <input type="hidden" name="barer_toekn" value="{{$token}}">
    <input type="hidden" name="amount" value="{{$refund->amount}}">

    <div class="modal-header bg-info">
        <h5 class="modal-title">You are going to release refund.</h5>
    ...
    <div class="modal-footer">
        ...
        <button type="submit" class="btn bg-primary">Release Refund</button>
    </div>
</form>
